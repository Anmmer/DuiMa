 ...
 */
public class ConstructionUnit extends HttpServlet {
    @Override
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            con = DbUtil.getCon();
            String sql1 = "select name,id from construction_unit where is_effective = 1";
            String sql1_page = "select count(*) as num from construction_unit where is_effective = 1";
            String sql2 = "insert into construction_unit values(?,?,now(),'1')";
            String sql3 = "update construction_unit set ";
            if ("1".equals(type)) {
                int pageCur = Integer.parseInt(req.getParameter("pageCur"));
                int pageMax = Integer.parseInt(req.getParameter("pageMax"));
                if (name != null && !"".equals(name)) {
                    sql1 += " and name like ?";
                    sql1_page += " and name like ?";
                    i++;
                }
                j = i;
                sql1 += " limit ?,?";
