...
<script type="text/javascript">
    ...
    function getTableData(newPage) {
        ...
        $.ajax({
            url: "${pageContext.request.contextPath}/FillingInfo",
            type: 'post',
            dataType: 'json',
            data: obj,
    ...
    function updateTable() {
        ...
        for (let i = 0; i < jsonObj.length; i++) {
            str += "<tr><td class='tdStyle_body'>" + jsonObj[i]['number'] +
                "</td><td class='tdStyle_body'><a href='#' onclick=openEditPop('" + jsonObj[i]['id'] + "','" + jsonObj[i]['number'] + "')>修改</a> <a href='#' onclick=delTableData('" + jsonObj[i]['id'] + "','" + jsonObj[i]['number'] + "')>删除</a></td></tr>";
        }
    ...
    function delTableData(id, planname) {
        ...
        $.post("${pageContext.request.contextPath}/FillingInfo", {type: '4', id: id}, function (result) {
    ...
    function save() {
        ...
        $.post("${pageContext.request.contextPath}/FillingInfo", obj, function (result) {
            result = JSON.parse(result);
            alert(result.message);
            if (result.flag) {
                $('#myModal').modal('hide');
